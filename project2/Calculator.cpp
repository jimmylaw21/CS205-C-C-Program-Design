// Calculator.cpp: Calculator类的主要实现文件
    ...
    case '!':
      if (symStack.empty() || symStack.top() == '(' || symStack.top() == '[' ||
        ...
        while (!symStack.empty() &&
               (getPrior(symStack.top()) >= getPrior(stdInfix[i]))) {
          tmp += symStack.top();
...
void Calculator::calResult() {
  ...
  for (int i = 0; i < postfix.size(); i++) {
    ...
    if (tmp[0] >= '0' && tmp[0] <= '9') {
      ...
      BigDecimal ZERO = 0;
      figStack.push(op1 > 0 ? op1 : ZERO - op1);
    } else if (postfix[i] == "!") {
      if (!figStack.empty()) {
        op1 = figStack.top();
        figStack.pop();
...
void Calculator::calculate() {
  ...
  if (isAssigning(infix)) {
    ...
    getPostfix(); // 后缀表达式转换
    calResult();  // 获取算术结果
    cout << getResult().toString() << endl;
  }
}

// 获取结果
BigDecimal Calculator::getResult() { return result; }
