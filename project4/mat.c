...
Mat *initMat(Mat *mat, int row, int col) {
  ...
  mat->col = col;
  // 动态分配二维数组的内存
  mat->data = (float *)aligned_alloc(256,sizeof(float) * row * col);
  return mat;
}
...
Mat *readMat_file(Mat *mat, int r, int c, char *filename){
  if (r <= 0 || c <= 0) {
    printf("The line or colomn input is improper!\n");
    return NULL;
  }
  ...
  mat = initMat(mat, r, c);
  // 给矩阵动态分配空间之后，读取相应的数据存储到矩阵中
  if (mat != NULL && mat->row == r && mat->col == c) {
    FILE *fp;
    fp = fopen(filename, "r");
    if (fp == NULL) {
      printf("File open failed!\n");
...
//获得以matsize为边长的矩阵的文件名，文件名为“”mat-A-”+matsize+“.txt”
char *get_mat_filename(int matsize) {
  char *filename = (char *)malloc(sizeof(char) * 100);
  char *matname = "/mnt/c/Users/jimmylaw21/OneDrive - 南方科技大学/桌面/CPP-main/project4/data/mat-A-";
  char *txtname = ".txt";
  char *size = (char *)malloc(sizeof(char) * 10);
  sprintf(size, "%d", matsize);
...
bool cpyMat(Mat *target, Mat *source) {
  if (isMatNull(source)||isMatNull(target))
  {
   return false; 
  }
  ...
  memcpy(target->data,source->data,source->row * source->col);
...
bool isMatNull(const Mat *mat) {
  // 检查指针
  if (mat == NULL) {
    printf("The pointer to the matrix is NULL!\n");
    return true;
  }
  // 检查矩阵是否为空
  if (mat->data == NULL) {
